...
const io = new Server(server, {
    cors: {
      origin: 'http://192.168.26.145:3000',
      methods: ['GET', 'POST'],
    },
  });
...
io.on('connection', (socket) => {
  ...
  socket.on("removeuser", function(data){
    ...
    var disUser = userConnections.find((p) => p.connectionId == data.userid);
      // var meetingid = disUser.meeting_id;
      // userConnections = userConnections.filter(
      //   (p) => p.connectionId != data.userid
      // );
      console.log(disUser);
      if (disUser) {
        var meetingid = disUser.meeting_id;
        console.log(userConnections);
        io.to(data.userid).emit("kickmeout", {});
        userConnections = userConnections.filter(
          (p) => p.connectionId != data.userid
        );
        console.log(userConnections);
        ...
        console.log('running inside list');
          console.log(v.connectionId);
