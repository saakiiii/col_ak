...
//   const socket = io('http://localhost:3001');
  ...
  useEffect(()=>{
    fetch("/api/check/access", {
      ...
      setFileowner(x.msg.created_by);
      // if (x.msg["accesstype"] == "Invite only"){
      //   confirm();
      // }
  ...
  // }, [socket])
  const removeUser = (connId, other_user_id) => {
    // Logic to handle the user removal
    if(confirm("Are you sure to kick out "+other_user_id+"?")){
    console.log(`User with connId: ${connId} removed.`);
    const userElement = document.getElementById(connId);
    // if (userElement) {
        // Remove the entire row or specific user element
        console.log("hi");
        socket.emit('removeuser', {userid:connId});
        // userElement.parentElement.remove(); // Removes the entire parent container
    // }
// }
    }
  }
  ...
  // start code for audio
    ...
    var MyApp = (function () {
      ...
      function event_process_for_signaling_server() {
        ...
        socket.on("inform_other_about_disconnected_user", function (data) {
          ...
          var temp = [...audios];
          console.log(temp);
          temp = temp.filter(x=>x.connId != data.connId);
          console.log(temp);
          setAudios(temp);
        ...
        socket.on("kickmeout", function(){
          alert("You have been kicked out");
          window.location.href = "/main/bases";
        });
        // <!-- .....................HandRaise .................-->
      ...
      function addUser(other_user_id, connId, userNum, username) {
        var temp = {other_user_id:other_user_id, connId:connId};
        setAudios([...audios, temp]);
    ...
    //     audioname.classList.add("flex", "items-center", "justify-center", "pt-2", "pb-2");
    //     audioname.innerHTML = `<div style="font-size:18px; font-weight:600; " class="w-[80%]">${other_user_id}</div><div id="${connId}" class="w-[20%] text-center" style="cursor: pointer;">
    //     x
    // </div>`;
        ...
        <div className='flex flex-col w-[45%] h-[98%] justify-between'>
          ...
          <div style ={{background:"rgb(14 21 39)"}} className='-[100%] h-[42%] mt-2 rounded overflow-y-scroll'>
            ...
            <h1 className='text-xl text-white p-3 font-bold' id={"cur_"+myConnId}>Collaborators</h1>
              ...
              <div id="audio-audio" className='pl-4 space-y-3 text-white'>
              {audios.map(x => (
              <React.Fragment key={x.connId}>
                <div>
                  <audio autoPlay controls style={{ display: "none" }} id={"a_" + x.connId}></audio>
                </div>
                  <div id={x.connId} className="flex items-center justify-center pt-2 pb-2">
                    <div style={{ fontSize: "18px", fontWeight: 600 }} className="w-[80%]">
                      {x.other_user_id}
                    </div>
                    {currentusername === fileowner ? (
                    ...
                    <div
                      ...
                      onClick={() => {
                        removeUser(x.connId, x.other_user_id);
                      }}
                    >
                      x
                    </div>
